...
\hline
\textbf{Semilla} & \textbf{JS} & \textbf{Wasserstein} & \textbf{Corr. err.} & \textbf{Autocorr.} & \textbf{Autocorr. abs.} & \textbf{Score} \\
\hline
...
Los diagnósticos distribucionales muestran además que la calidad de una cartera no puede juzgarse únicamente por el Sharpe final. Preservar la estructura de correlaciones es esencial porque la diversificación depende precisamente de cómo se relacionan los activos entre sí. Del mismo modo, las colas de la distribución, el VaR, el CVaR y la autocorrelación aportan información que las métricas medias no capturan. Esto explica por qué un escenario puede parecer plausible a simple vista y, sin embargo, no ser suficientemente fiel para apoyar decisiones de asignación de capital.

En ese sentido, el valor metodológico del trabajo está más en el protocolo que en una cifra aislada. Separar entrenamiento, validación y prueba, registrar metadatos de ejecución, comparar varias semillas y evaluar frente a baselines clásicos robustos reduce la probabilidad de sobreinterpretar un resultado puntual. En modelos generativos aplicados a finanzas, esta disciplina experimental no es opcional: es la única forma razonable de distinguir una mejora real de un buen resultado circunstancial.

También conviene subrayar las limitaciones del piloto. El universo analizado es reducido, la frecuencia es diaria, el periodo histórico es único y no se incluyen costes de transacción, impacto de mercado ni validación en un entorno real de decisión. Estas restricciones no invalidan el estudio, pero sí acotan su alcance. La conclusión correcta no es que el enfoque generativo falle, sino que todavía necesita más evidencia para competir con garantías frente a métodos clásicos robustos.
...
Aunque el sistema desarrollado en este trabajo tiene un carácter experimental, es razonable describir una evolución futura que lo acerque a un prototipo utilizable por otros investigadores o usuarios internos. La idea no es automatizar decisiones de inversión, sino convertir el pipeline en una aplicación que permita ejecutar experimentos de forma controlada, revisar datos, comparar modelos y conservar trazabilidad sobre cada ejecución.
...
En síntesis, la propuesta de puesta en producción debe entenderse como una continuación natural del trabajo: no como un sistema de inversión automático, sino como un prototipo web reproducible para ejecutar el pipeline, revisar resultados y comparar enfoques de forma ordenada.
