...
En particular se llevaron a cabo las siguientes acciones para el análisis y limpieza de los datos.

En primer lugar, se comprueba la presencia de valores ausentes en el conjunto de datos, añadiendo además la función dropna() para eliminar aquellas filas que contengan valores de este tipo. Tras ejecutar dicha función y consultar los estadísticos confirmamos que no existen valores nulos en la extracción del dataset inicial, al tratarse de series financieras procedentes de una fuente que habitualmente proporciona datos ya alineados y sin huecos. Esto se termina de confirmar tras el uso de la función mencionada, que no eliminó ninguna fila, manteniéndose el mismo número de registros antes y después de su ejecución.

En segundo lugar,  se analiza la evolución temporal de los precios seleccionados durante el periodo de estudio y se analiza la posible presencia de estructura temporal en las series para observar el comportamiento histórico de cada serie y detectar aspectos relevantes antes de transformar los precios.

Como se observa en la figura \ref{fig:eda-precios} se aprecia una tendencia creciente en la mayoría de los activos, aunque con diferencias relevantes entre ellos. En concreto los activos tecnológicos como META,MSFT, GOOGL, AAPL y NVDA, presentan un crecimiento más acusado a los largo del tiempo, especialmente desde 2020. Algunos activos como por ejemplo, META, presentan episodios de inestabilidad y fuertes caídas con una repercusión muy marcada en los años siguientes, lo que demuestra que las series financieras no evolucionan de forma estable o lineal, sino que pueden presentar períodos de grandes cambios. A diferencia de los ya comentados, se puede ver reflejado que activos como JNJ, JPM o XOM muestran una evolución más moderada en términos de precio.

Estas diferencias entre ellos justifican la necesidad de analizar la relación que existe y no sólo la rentabilidad individual ya que la optimización de carteras depende de la estructura conjunta de retornos, volatilidades y correlaciones.

La gráfica también sugiere la posible presencia de estructura temporal en las series, los precios no se mueven de forma aleatoria e independiente, sino que presentan tendencias sostenidas en el tiempo. Esta dependencia se debe tener muy en cuenta a la hora de entrenar los modelos generativos considerados, ya que deben ser capaces de ir más allá de reproducir la distribución de los retornos en un momento dado y capturar también la dinámica con la que estos evolucionan a lo largo del tiempo.
...
En tercer lugar se consideran los precios ajustados descargados directamente de la fuente de datos y los retornos logarítmicos calculados a partir de dichos datos para analizar los estadísticos descriptivos antes de aplicar la limpieza final del pipeline, en este análisis se incluyen datos relevantes como la media de los precios, el precio máximo y mínimo o la asimetría que presentan.

Como se muestra en la Tabla~\ref{tab:eda-estadisticos-origen}, las medias de los retornos logarítmicos se sitúan próximas a cero en todos los activos, algo habitual en series financieras diarias ya que la tendencia media de crecimiento esperada en un único día es prácticamente despreciable frente a la volatilidad del mercado. Sin embargo, existen diferencias importantes en la desviación típica con las que se deduce una gran volatilidad, activos como NVDA, META o AMZN presentan una mayor dispersión en sus retornos, mientras que otros como JNJ o PG muestran una variabilidad más reducida.

También resultan relevantes los valores mínimo y máximo de los retornos, ya que permiten identificar la existencia de movimientos diarios extremos y la asimetría y la curtosis que permiten caracterizar la forma de la distribución empírica y apreciar con mayor detalle la probabilidad de observar eventos extremos respecto a una distribución normal .En este contexto se observa que activos como META y NVDA presentan los movimientos diarios más extremos, mientras que NVDA muestra la mayor desviación típica de los retornos, reflejando un comportamiento más volátil.
...
Para continuar con el análisis de los datos, en cuarto lugar, se genera la matriz de correlación de los retornos logarítmicos con el objetivo de estudiar la relación existente entre los activos seleccionados.

Como se observa en la Figura~\ref{fig:eda-matriz-correlacion}, todas las correlaciones son positivas, aunque con intensidades diferentes según el tipo de activo.

Se aprecia que los activos tecnológicos presentan correlaciones relativamente elevadas entre sí. Por ejemplo, MSFT, GOOGL, AMZN, META, NVDA y AAPL muestran valores de correlación moderados o altos, lo que indica que tienden a moverse en la misma dirección en muchos periodos del mercado. Este comportamiento resulta coherente, ya que pertenecen a sectores próximos y pueden verse afectados por factores comunes, como expectativas sobre crecimiento tecnológico, tipos de interés o sentimiento general del mercado.

Por otro lado, activos como XOM, JNJ o PG presentan correlaciones más bajas con las grandes tecnológicas. Esto sugiere que aportan cierto grado de diversificación dentro de la cartera, al no comportarse exactamente igual que los activos más orientados al crecimiento. En particular, se observa una relación más intensa entre JNJ y PG, ambos vinculados a sectores más defensivos, y entre JPM y XOM, lo que refleja cierta proximidad en su comportamiento respecto al resto de activos no tecnológicos.

Este análisis es especialmente relevante para nuestro trabajo ya que modelos como mínima varianza y Markowitz dependen directamente de la matriz de covarianzas y una estimación inestable de estas relaciones puede afectar de forma significativa a los pesos finales de la cartera.Es por ello que en el trabajo se incorpora posteriormente el estimador Ledoit-Wolf, con el objetivo de obtener una estimación más robusta de la matriz de covarianzas.
...
En quinto lugar, se analiza la distribución empírica de los retornos logarítmicos de los activos seleccionados, mediante los correspondientes histogramas, para estudiar si estos siguen una distribución normal o presentan desviaciones relevantes como colas o asimetría. 

Como se observa en la Figura~\ref{fig:eda-histogramas}, todos los activos presentan distribuciones concentradas alrededor de cero, lo cual es habitual en series financieras diarias debido a que la mayor parte de las variaciones diarias de precio son moderadas, mientras que los movimientos de mayor magnitud aparecen con menor frecuencia.

Aunque los histogramas presentan una forma general similar, existen diferencias relevantes entre activos, ya que META, NVDA o AMZN muestran una mayor dispersión y colas más amplias, lo que refleja una mayor probabilidad de observar variaciones diarias extremas, mientras que JNJ o PG presentan distribuciones más estrechas y concentradas en torno a cero, sugiriendo un comportamiento más estable y defensivo dentro del conjunto analizado. 

La forma de estas distribuciones y la presencia de colas pronunciadas y valores alejados del centro permite apreciar que los retornos financieros no siguen un comportamiento perfectamente normal existiendo episodios de alta volatilidad y movimientos extremos, esto presenta un reto para el actual trabajo ya que si un modelo no reproduce adecuadamente la distribución empírica de los retornos, especialmente en sus extremos, las métricas de riesgo y las decisiones de asignación de pesos podrían verse afectadas.
...
Por último, la Figura~\ref{fig:eda-division-temporal} representa la división temporal de los datos en los subconjuntos de entrenamiento, validación y test, partición que se realiza respetando el orden cronológico de las observaciones de forma que el entrenamiento corresponde al primer tramo temporal, la validación a un periodo posterior y el test al tramo final, como ya explicamos en el apartado anterior.

Esta separación es muy importante en series financieras, ya que una división aleatoria podría introducir fuga de información al permitir que datos futuros influyeran en el entrenamiento o la selección de los modelos, por lo que el uso de una partición temporal permite simular un escenario más realista en el que las estrategias se construyen con información pasada y se evalúan sobre datos posteriores no vistos durante el entrenamiento. 

Además, la figura permite observar que el periodo de test se sitúa en una etapa reciente del mercado caracterizada por un crecimiento acusado y una gran variabilidad, lo que refuerza la necesidad de evaluar los modelos sobre un tramo temporal separado dado que el comportamiento del mercado puede diferir notablemente entre los periodos de entrenamiento, validación y prueba. 
...
El análisis llevado a cabo permite es fundamental antes de aplicar cualquier tipo de modelo ya que nos ha permitido obtener una visión completa del comportamiento inicial de los datos financieros utilizados en el trabajo, conociendo su estructura y calidad de forma que permite justificar decisiones posteriores como el uso de retornos logarítmicos, la división temporal en entrenamiento, validación y test o la comparación mediante métricas estadísticas.
...
En las variantes robustas incorporadas en este trabajo, la matriz $\Sigma$ no se estima mediante la covarianza muestral histórica sino mediante el estimador Ledoit-Wolf \cite{LedoitWolf2004}, que aplica un proceso de regularización sobre la matriz de covarianzas con el objetivo de obtener una estimación mejor condicionada y menos sensible al ruido de los datos históricos. 

La idea de este estimador consiste en combinar la covarianza muestral con una matriz objetivo más estable, ponderando ambas mediante un coeficiente de contracción estimado de forma óptima y reduciendo la influencia de los valores extremos presentes en la muestra. 

El objetivo del mismo se cumple si se obtiene una matriz que tiende a ser más estable cuando el número de activos es elevado en relación con el número de observaciones disponibles, una situación que resulta común en problemas de optimización de carteras.

Las carteras \texttt{ledoit\_wolf\_minimum\_variance} y \texttt{ledoit\_wolf\_markowitz} mantienen las mismas restricciones de optimización que sus equivalentes clásicos, pero parten de esta estimación más robusta del riesgo conjunto entre activos. Esta distinción resulta relevante porque una matriz de covarianzas inestable puede generar pesos excesivamente sensibles a pequeñas variaciones en los retornos históricos, comprometiendo el comportamiento de la cartera fuera de muestra. 

La incorporación de estas variantes permite así reforzar los baselines financieros y plantear una comparación más exigente frente a las carteras basadas en escenarios sintéticos, ya que no basta con superar versiones clásicas potencialmente frágiles, sino también métodos que incorporan una gestión más cuidadosa de la estimación del riesgo.
...
Como ya hemos comentado anteriormente, además de los baselines clásicos iniciales, se incorporaron dos variantes robustas basadas en la estimación de covarianza Ledoit-Wolf \cite{LedoitWolf2004}, en concreto \texttt{ledoit\_wolf\_minimum\_variance} y \texttt{ledoit\_wolf\_markowitz}. con el objetivo de reforzar la comparación financiera mediante una estimación más estable de la matriz de covarianzas, manteniendo las mismas restricciones del experimento.

La Tabla~\ref{tab:resultados-ledoit-wolf} muestra la comparación entre los modelos clásicos basados en covarianza muestral y sus equivalentes robustos con Ledoit-Wolf.

{\small
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.2}
\begin{longtable}{|p{0.31\textwidth}|p{0.13\textwidth}|p{0.13\textwidth}|p{0.10\textwidth}|p{0.14\textwidth}|}
...
Los resultados muestran que la variante Ledoit-Wolf de mínima varianza mejora ligeramente a la cartera de mínima varianza clásica, con un aumento de la rentabilidad anualizada de 13,50\% a 13,62\%, una reducción de la volatilidad de 12,81\% a 12,73\% y una mejora del ratio de Sharpe de 1,05 a 1,07.

Las diferencias no son muy elevadas, lo cual era esperable ya que cuando la muestra histórica es suficientemente larga y los activos no son demasiado numerosos, la covarianza muestral ya ofrece una estimación razonablemente estable, dejando poco margen para que la introducir mejoras. 

En el caso de Markowitz, el efecto es más limitado. La variante robusta obtiene una rentabilidad anualizada algo superior, pasando de 22,94\% a 23,20\%, pero mantiene una volatilidad y un drawdown prácticamente iguales a los del modelo clásico. Esto sugiere que, en esta configuración experimental, la sensibilidad de Markowitz no depende únicamente de la matriz de covarianzas sino también de la estimación de los retornos esperados, que permanece inalterada en ambas variantes. 

Por tanto, la incorporación de Ledoit-Wolf no pretende transformar de forma radical los baselines, sino reforzar la comparación metodológica de forma los modelos generativos deban medirse no sólo frente a versiones clásicas , sino también frente a variantes que incorporan una estimación más cuidadosa del riesgo.
...
Aunque el sistema desarrollado en este trabajo tiene un carácter experimental resulta conveniente plantear cómo podría evolucionar y avanzar hacia una versión más accesible por parte de los distintos usuarios interesados en él. En esta línea, se plantea como posible propuesta el desarrollo de una web que permita ejecutar el pipeline desarrollado de forma controlada, visualizar los resultados obtenidos y facilitar la comparación de modelos clásicos, robustos y generativos sin necesidad de interactuar directamente con el código.

La idea principal consiste en transformar el pipeline actual, consistente en distintos scripts de Python, en una plataforma web sencilla que permita centralizar en una interfaz única las principales fases de nuestro trabajo: la descarga de datos, el análisis exploratorio, la generación de escenarios sintéticos, la evaluación y optimización de carteras y una comparación final de resultados. De esta forma, se obtiene una interfaz donde un usuario puede seleccionar un conjunto de activos, definir un periodo temporal, ejecutar el análisis y consultar los resultados obtenidos.

Desde el punto de vista funcional, la aplicación podría organizarse como un panel web dividido en varias secciones o módulos que quedan resumidas a continuación en forma de tabla:
...
La arquitectura técnica propuesta sería sencilla y modular, con un backend desarrollado en FastAPI por su capacidad para exponer endpoints REST de forma clara y su buena integración con código Python existente, y un frontend que nos permita construir paneles interactivos rápidamente reutilizando los gráficos y resultados generados por el pipeline sin necesidad de desarrollar una capa de presentación desde cero. 

El flujo de uso sería sencillo: el usuario seleccionaría los activos y el periodo de análisis, tras lo cual el sistema descargaría automáticamente los precios ajustados, verificaría la calidad de los datos y calcularía los retornos logarítmicos mostrando el análisis exploratorio correspondiente. Una vez revisada esta fase, el usuario podría lanzar la comparación de baselines, obteniendo los pesos de cada cartera, su evaluación sobre el conjunto de test y una tabla comparativa con las métricas principales.

En una versión inicial, la aplicación podría ejecutarse localmente o desplegarse en un servidor sencillo, como una máquina virtual o un servicio cloud básico,  sin necesidad de una infraestructura compleja.

Por último, esta propuesta debe mantenerse alineada con el carácter experimental del trabajo, el objetivo no es automatizar decisiones de inversión sino ofrecer una herramienta que permita ejecutar el pipeline, revisar los datos y comparar estrategias bajo criterios controlados.

No obstante, esta propuesta debe entenderse como una primera aproximación orientada a hacer el pipeline más accesible y reproducible con el objetivo principal de transformar el trabajo desarrollado en un prototipo utilizable desde el que ejecutar experimentos, visualizar resultados y comparar modelos de forma ordenada. De cara al futuro se podría desarrollar otra versión que contenga mejoras notables como incorporar fuentes de datos adicionales y mecanismos de actualización periódica o añadir una gestión más completa de los modelos entrenados que permitiera almacenar configuraciones, registrar semillas y repetir experimentos anteriores de forma exacta.
