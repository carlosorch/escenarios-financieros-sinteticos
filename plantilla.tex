...
Una vez presentados los conceptos fundamentales de la inteligencia artificial y los modelos generativos, nos adentramos en el otro gran pilar de este trabajo: la optimización de carteras y la teoría moderna de carteras, un ámbito que, aunque parezca que se aleja del terreno de la IA, resulta igualmente esencial para entender el enfoque propuesto. 
...
La optimización de carteras puede definirse como el proceso de seleccionar la combinación más adecuada de activos financieros dentro de una cartera, asignando pesos a cada activo con el objetivo de encontrar el mejor equilibrio posible entre lo que se espera ganar y el riesgo que se está dispuesto a asumir. 
...
La partición de los datos será temporal y no aleatoria: entrenamiento entre 2015 y 2022, validación desde enero de 2023 hasta junio de 2024 y prueba desde julio de 2024 hasta el 12 de mayo de 2026. Las ventanas temporales tendrán una longitud inicial de 30 días por el número de activos seleccionados, es decir, 30 días por 10 activos en el universo experimental propuesto.

\section{Modelado generativo}

El núcleo técnico del trabajo será la comparación entre un VAE utilizado como baseline generativo probabilístico y TimeGAN como modelo generativo temporal principal. Ambos modelos se entrenarán sobre las mismas ventanas temporales y se evaluarán con un protocolo común, de forma que la comparación no dependa de diferencias en la preparación de datos o en el conjunto de evaluación. Los modelos de difusión se consideran una línea reciente y prometedora dentro del estado del arte, pero quedan planteados como posible extensión si el alcance temporal del proyecto lo permite.
...
Los modelos no generarán precios directamente, sino retornos sintéticos. La salida esperada tendrá la forma de \texttt{N escenarios x T días x M activos}; por ejemplo, 1000 escenarios de 30 días para 10 activos. Estos escenarios se utilizarán para comparar distribuciones reales y sintéticas, estimar parámetros de retorno y riesgo, construir carteras mediante modelos de optimización y analizar las asignaciones obtenidas mediante estabilidad de pesos, concentración, entropía y comportamiento fuera de muestra.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.18\textwidth}|p{0.28\textwidth}|p{0.22\textwidth}|}
...
Los baselines financieros serán la cartera equiponderada, Markowitz media-varianza con datos históricos reales, la cartera de mínima varianza, la cartera optimizada usando escenarios VAE y la cartera optimizada usando escenarios TimeGAN. Los escenarios sintéticos se emplearán para estimar medias, covarianzas o distribuciones de retornos y construir carteras long-only, completamente invertidas y sin ventas en corto, pero la comparación de rendimiento se realizará siempre sobre retornos reales del conjunto de prueba.
...
\section{Ejecución del pipeline experimental}

Tras definir el protocolo metodológico, se implementó una primera versión reproducible del pipeline experimental en Python. El flujo descarga precios ajustados desde Yahoo Finance, calcula retornos logarítmicos diarios, aplica la partición temporal definida en la metodología y entrena modelos generativos VAE y TimeGAN sobre ventanas multiactivo de 30 días. La evaluación financiera se realiza siempre sobre retornos reales del conjunto de prueba, mientras que los escenarios sintéticos se utilizan únicamente para estimar información de media, covarianza o distribución antes de construir las carteras.
...
\section{Resultados financieros fuera de muestra}

La Tabla~\ref{tab:resultados-baselines} resume los principales baselines clásicos evaluados sobre el periodo de prueba. La cartera equiponderada obtuvo un ratio de Sharpe de 1,14, mientras que la cartera de mínima varianza alcanzó 1,05 con menor volatilidad y menor drawdown. El modelo de Markowitz histórico obtuvo mayor rentabilidad anualizada, pero con una volatilidad sustancialmente superior y un Sharpe inferior.

\begin{longtable}{|p{0.24\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.11\textwidth}|p{0.13\textwidth}|}
...
En la evaluación multi-semilla de TimeGAN, los resultados muestran un comportamiento competitivo, aunque no una superioridad robusta frente a los baselines. La variante TimeGAN sin calibrar usada para mínima varianza obtuvo un Sharpe medio de 1,22 con desviación típica de 0,55. La variante calibrada a media y volatilidad para mínima varianza obtuvo un Sharpe medio de 1,09 con desviación típica de 0,57. Por su parte, la cartera con media histórica contraída y covarianza sintética calibrada obtuvo un Sharpe medio de 1,13 con desviación típica de 0,66. Estos valores sugieren que los escenarios generados pueden aportar información útil para estimar riesgo, pero también muestran una sensibilidad relevante a la semilla de entrenamiento.

\begin{longtable}{|p{0.30\textwidth}|p{0.16\textwidth}|p{0.16\textwidth}|p{0.16\textwidth}|}
...
La comparación con la cartera equiponderada es especialmente relevante: aunque algunas variantes TimeGAN alcanzan Sharpe medio similar o ligeramente superior, la desviación típica entre semillas es alta. Por tanto, no resulta adecuado afirmar que TimeGAN supere de forma robusta a los enfoques clásicos en esta fase preliminar. Una lectura más prudente es que TimeGAN produce estimaciones de covarianza que pueden ser competitivas en determinadas configuraciones, pero cuya estabilidad debe analizarse con mayor profundidad.

\section{Fidelidad distribucional y limitaciones}
...
\section{Interpretación provisional}

En conjunto, los experimentos iniciales permiten extraer tres conclusiones provisionales. En primer lugar, el pipeline técnico es reproducible y permite comparar baselines clásicos, VAE y TimeGAN bajo el mismo protocolo de datos y evaluación. En segundo lugar, la evaluación fuera de muestra confirma que algunas carteras construidas con información sintética pueden comportarse de forma competitiva frente a baselines clásicos, especialmente cuando los escenarios se usan para estimar covarianzas y no para depender directamente de medias sintéticas inestables. En tercer lugar, la sensibilidad entre semillas y los errores de correlación y curtosis obligan a mantener una interpretación cautelosa y a reforzar la validación antes de formular conclusiones definitivas.
